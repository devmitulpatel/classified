...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.messageBox.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.message-boxes.update", [$messageBox->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users_id" id="users_id" required>
                    @foreach($users as $id => $users)
                        <option value="{{ $id }}" {{ (old('users_id') ? old('users_id') : $messageBox->users->id ?? '') == $id ? 'selected' : '' }}>{{ $users }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from') ? 'is-invalid' : '' }}" name="from_id" id="from_id" required>
                    @foreach($froms as $id => $from)
                        <option value="{{ $id }}" {{ (old('from_id') ? old('from_id') : $messageBox->from->id ?? '') == $id ? 'selected' : '' }}>{{ $from }}</option>
                    @endforeach
                </select>
                @if($errors->has('from'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message" required>{{ old('message', $messageBox->message) }}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\MessageBox::READ_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('read_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="read_status_{{ $key }}" name="read_status" value="{{ $key }}" {{ old('read_status', $messageBox->read_status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\MessageBox::DELIVER_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('deliver_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="deliver_status_{{ $key }}" name="deliver_status" value="{{ $key }}" {{ old('deliver_status', $messageBox->deliver_status) === (string) $key ? 'checked' : '' }}>
