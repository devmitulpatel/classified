...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.paymentForAdmin.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-for-admins.update", [$paymentForAdmin->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('method') ? 'is-invalid' : '' }}" name="method_id" id="method_id">
                    @foreach($methods as $id => $method)
                        <option value="{{ $id }}" {{ (old('method_id') ? old('method_id') : $paymentForAdmin->method->id ?? '') == $id ? 'selected' : '' }}>{{ $method }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $paymentForAdmin->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $paymentForAdmin->amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('plan') ? 'is-invalid' : '' }}" name="plan_id" id="plan_id">
                    @foreach($plans as $id => $plan)
                        <option value="{{ $id }}" {{ (old('plan_id') ? old('plan_id') : $paymentForAdmin->plan->id ?? '') == $id ? 'selected' : '' }}>{{ $plan }}</option>
                    @endforeach
                </select>
                @if($errors->has('plan'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ref_products') ? 'is-invalid' : '' }}" name="ref_products[]" id="ref_products" multiple>
                    @foreach($ref_products as $id => $ref_product)
                        <option value="{{ $id }}" {{ (in_array($id, old('ref_products', [])) || $paymentForAdmin->ref_products->contains($id)) ? 'selected' : '' }}>{{ $ref_product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ref_service') ? 'is-invalid' : '' }}" name="ref_service_id" id="ref_service_id">
                    @foreach($ref_services as $id => $ref_service)
                        <option value="{{ $id }}" {{ (old('ref_service_id') ? old('ref_service_id') : $paymentForAdmin->ref_service->id ?? '') == $id ? 'selected' : '' }}>{{ $ref_service }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by') ? 'is-invalid' : '' }}" name="approved_by_id" id="approved_by_id">
                    @foreach($approved_bies as $id => $approved_by)
                        <option value="{{ $id }}" {{ (old('approved_by_id') ? old('approved_by_id') : $paymentForAdmin->approved_by->id ?? '') == $id ? 'selected' : '' }}>{{ $approved_by }}</option>
                    @endforeach
                </select>
