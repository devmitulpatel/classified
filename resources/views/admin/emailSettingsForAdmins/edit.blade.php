...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.emailSettingsForAdmin.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.email-settings-for-admins.update", [$emailSettingsForAdmin->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.emailSettingsForAdmin.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $emailSettingsForAdmin->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" name="value" id="value" required>{{ old('value', $emailSettingsForAdmin->value) }}</textarea>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('display_type') ? 'is-invalid' : '' }}" name="display_type" id="display_type" required>
                    ...
                    @foreach(App\Models\EmailSettingsForAdmin::DISPLAY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('display_type', $emailSettingsForAdmin->display_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('store_type') ? 'is-invalid' : '' }}" name="store_type" id="store_type" required>
                    ...
                    @foreach(App\Models\EmailSettingsForAdmin::STORE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('store_type', $emailSettingsForAdmin->store_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
